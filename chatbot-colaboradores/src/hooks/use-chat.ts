  ...
  try {
    ...
    for (const legacyKey of opportunityKeys) {
      ...
      try {
        ...
        // Strict mode: use only resources for this exact competency and pick at most one.
        let selected: ResourceRecommendation | null = null;

        for (const row of rows) {
          const rowCompetencyId = String(row.competency_id ?? "").trim().toLowerCase();
          if (rowCompetencyId !== normalizedCatalogId) continue;
        ...
        if (selected) {
          const key = normalizeTitleKey(selected.title);
          if (key && !seenTitles.has(key)) {
            seenTitles.add(key);
            recommendedResources.push(selected);
          }
        }
      } catch {
